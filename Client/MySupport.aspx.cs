...
public partial class MyDetails : Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            FillSupportDetails();
            //Show title
            Title = "Client Panel :: My Domains";

            //Create Bread Crumbs
            lblBreadCrumb.Text = string.Format("<a href=\'{0}\'>Home</a>  &gt; <a href=\'{1}\'>My Support</a>",
                ResolveUrl("~"), ResolveUrl("~/MySupport.aspx"));
        }
    }
    ...
    private void FillSupportDetails()
    {
        ...
        if (ds != null)
        {
            ...
            {
                var intRows = ds.Tables["GetSupportDetails"].Rows.Count;
    ...
    protected void gvSupport_RowCommand(object sender, GridViewCommandEventArgs e)
    {
            ...
            if (dsMessages.Tables["GetSupportDetails"].Rows.Count > 0)
            {
                foreach (DataRow msgRow in dsMessages.Tables["GetSupportDetails"].Rows)
                {
                    ...
                    lblMessage.Text = string.Format("<strong>Subject : </strong>{0}<br /><br /><br />{1}",
                        CleanUtils.ToString(msgRow["Subject"]), CleanUtils.ToString(msgRow["Query"]));

                    //Change BreadCrumbs

                    lblBreadCrumb.Text =
                        string.Format("<a href=\'{0}\'>Home</a>  &gt; <a href=\'{1}\'>My Support</a>  &gt; {2}",
                            ResolveUrl("~"), ResolveUrl("~/MySupport.aspx"), CleanUtils.ToString(msgRow["Subject"]));
                }
            }
        }
        if (e.CommandName == "Reopen")
        {
            ...
            foreach (DataRow msgRow in dsMessages.Tables["GetSupportDetails"].Rows)
            {
                ...
                txtReopen.Text =
                    string.Format(
                        "<br /><hr /><strong>Last Updated : {0} on Date : {1}</strong><br /><font color=\"blue\">{2}</font>",
                        CleanUtils.ToString(msgRow["Staff Name"]), CleanUtils.ToString(msgRow["UpdatedDate"]),
                        CleanUtils.ToString(msgRow["Query"]));
                txtReopen.Focus();
            }
            ...
            lblBreadCrumb.Text =
                string.Format("<a href=\'{0}\'>Home</a>  &gt; <a href=\'{1}\'>My Support</a>  &gt; Reopen",
                    ResolveUrl("~"), ResolveUrl("~/MySupport.aspx"));
        }
    }
    ...
    protected void btnRaise_Click(object sender, EventArgs e)
    {
        ...
        //Create Bread Crumbs
        lblBreadCrumb.Text =
            string.Format("<a href=\'{0}\'>Home</a>  &gt; <a href=\'{1}\'>My Support</a>  &gt; Raise Ticket",
                ResolveUrl("~"), ResolveUrl("~/MySupport.aspx"));
    }
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        ...
        var strKey = CleanUtils.GenerateTicketId();
        bool blnRes = objMsDnH.GenerateTicket(strKey, Convert.ToInt32(Session["UserID"]), rbDept.SelectedValue,
            ddlPriority.SelectedValue, txtPanelDetail.Text, txtACCID.Text, txtSubject.Text, txtQuery.Text, "Open",
    ...
    protected void btnReSubmit_Click(object sender, EventArgs e)
    {
        ...
        bool blnRes = objMsDnH.UpdateTicket(CleanUtils.ToString(ViewState["id"]), txtReopen.Text, "Reopen",
            Convert.ToInt32(Session["UserID"]));
        if (blnRes)
        {
            //Reload grid
